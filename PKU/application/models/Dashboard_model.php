...
class Dashboard_model extends CI_Model {
	...
	public function select_t_pelatihan()
        {
                $query = $this->db->query("select count(*) as pelatihan_selesai from T_PELATIHAN where STATUS = 'lpj_approved' and CABANG_ULAMM in (SELECT KODE_CABANG_REGION FROM MS_USER_CABANG_REGION WHERE ID_USER=".$this->session->userdata('sess_user_id')." and AKTIF=1 ) ");
        ...
        {
                $query = $this->db->query("select count(*) as jumlah_non_nasabah from T_KEHADIRAN where NASABAH_TIPE = 'NON_NASABAH' and ID_PELATIHAN in (SELECT ID FROM T_PELATIHAN WHERE CABANG_ULAMM in (SELECT KODE_CABANG_REGION FROM MS_USER_CABANG_REGION WHERE ID_USER=".$this->session->userdata('sess_user_id')." and AKTIF=1 ) ) ");
                return $query->row()->jumlah_non_nasabah;
        }

        public function select_nasabah_ulamm()
        {
                $query = $this->db->query("select count(*) as jumlah_kehadiran from T_KEHADIRAN where BISNIS = 'ULAMM' and ID_PELATIHAN in (SELECT ID FROM T_PELATIHAN WHERE CABANG_ULAMM in (SELECT KODE_CABANG_REGION FROM MS_USER_CABANG_REGION WHERE ID_USER=".$this->session->userdata('sess_user_id')." and AKTIF=1 ) ) ");
                return $query->row()->jumlah_kehadiran;
        }        

        public function select_nasabah_mekaar()
        {
                $query = $this->db->query("select count(*) as jumlah_kehadiran from T_KEHADIRAN where BISNIS = 'MEKAAR' and ID_PELATIHAN in (SELECT ID FROM T_PELATIHAN WHERE CABANG_ULAMM in (SELECT KODE_CABANG_REGION FROM MS_USER_CABANG_REGION WHERE ID_USER=".$this->session->userdata('sess_user_id')." and AKTIF=1 ) ) ");
                return $query->row()->jumlah_kehadiran;
        }        
